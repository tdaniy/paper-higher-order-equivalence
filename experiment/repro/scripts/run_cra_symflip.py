#!/usr/bin/env python3
"""Run CRA symflip diagnostic to isolate the N^{-1/2} term."""
...
def run_cra_symflip(
    ...
    n_grid = cfg["n_grid"]
    f_grid = list(cfg.get("f_grid", []))
    near_census = list(cfg.get("near_census_c", []))
    if not f_grid and "f" in cfg:
        f_grid = [float(cfg["f"])]
    if not f_grid and not near_census:
        raise ValueError("Provide f_grid, near_census_c, or f in cra_symflip config.")
    ...
    for n in n_grid:
        f_vals = list(f_grid)
        for c in near_census:
            f_nc = 1.0 - float(c) / math.sqrt(n)
            if 0.0 < f_nc < 1.0:
                f_vals.append(round(f_nc, 6))
        f_vals = sorted(set(f_vals))
        f_vals_by_n[n] = f_vals
        total_steps += len(f_vals)

    start_ts = time.time()
    start_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_ts))
    print(f"[cra_symflip] START {start_str} | steps={total_steps}", flush=True)

    spawn_keys = []
    step_idx = 0
    for n in n_grid:
        for f in f_vals_by_n[n]:
            step_idx += 1
            print(f"[cra_symflip] step {step_idx}/{total_steps} | N={n} f={f:.4f}", flush=True)
    ...
    print(f"[cra_symflip] DONE {end_str} | total_time={total_sec:.1f}s", flush=True)

    paths = build_run_paths(repro_root, "cra_symflip", run_id)
    ...
    append_summary(
        os.path.join(repro_root, "outputs", "cra_symflip", "summary.csv"),
        [
            run_id,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CRA symflip diagnostic module")
...
def main() -> None:
    ...
    master_rows = run_cra_symflip(cfg["cra_symflip"], seed, run_id, repro_root, config_path, alpha)
