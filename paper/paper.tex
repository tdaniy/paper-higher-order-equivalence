...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{../experiment/kill_shot_rate_grid_tight_sqrt_log.png}
\vspace{0.6em}
\includegraphics[width=0.95\linewidth]{../experiment/kill_shot_rate_grid_tight_linear_log.png}
\caption{Stress-test coverage errors for skewed lognormal outcomes under CRA at $p\in\{0.5,0.9\}$. Top: $\sqrt{N}$-scaled error; bottom: $N$-scaled error. Shaded bands are Monte Carlo standard errors. Near census ($p=0.9$), the Gaussian and Cornish--Fisher errors align with $O(N^{-1/2})$ scaling, while the calibrated interval reduces error to the simulation noise floor.}
