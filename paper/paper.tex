...
\noindent The paper makes three specific contributions:
\begin{itemize}[leftmargin=*,itemsep=2pt]
\item It establishes Le Cam deficiency-distance equivalence for finite-population randomization experiments.
\item It shows CLT-order sufficiency of $(\hat\tau,\hat V)$ and the resulting pivot geometry under reduced information.
\item It isolates sampling-fraction skew and parity-driven regimes, with extensions to lattice outcomes and clustered designs.
\end{itemize}
...
Our main contribution is an experiment-level approximation: the finite-population randomization experiment is asymptotically equivalent, in Le Cam deficiency distance, to a Gaussian shift experiment. This implies asymptotic sufficiency of $(\hat\tau,\hat V)$ at CLT order, so any regular design-valid equal-tailed Bayesian procedure measurable with respect to $\sigma(\hat\tau,\hat V)$ must agree with the Gaussian pivot up to $o_p(m_N^{-1/2})$. The remaining sections develop higher-order refinements (sampling-fraction skew and parity conditions) and extensions (clustered designs and lattice outcomes).
...
skew terms. If $p_N\to 1$ (or $0$) and $c_\Delta(p_N)\to\infty$, then the sampling-fraction term scales as
...
under these local alternatives. In particular, $(\hat\tau,\hat V)$ is asymptotically sufficient at CLT order.
\end{theorem}
...
The results provide a principled foundation for \emph{Objective Bayes} in randomized experiments. Rather than treating a reference posterior as a computational convenience, our findings identify a canonical Bayesian object within the scope of the randomization model: repeated-randomization validity constrains posterior geometry to coincide with the Gaussian pivot at CLT order, and it determines the allowable higher-order corrections.
