...
\begin{abstract}
We study inference for average treatment effects under finite-population randomization. Our main result establishes Le Cam deficiency-distance equivalence between the randomization experiment and a Gaussian shift experiment, yielding CLT-order sufficiency of $(\hat\tau,\hat V)$. Consequently, any regular design-valid equal-tailed Bayesian interval measurable with respect to $(\hat\tau,\hat V)$ coincides with the studentized Gaussian pivot up to $o_p(m_N^{-1/2})$. We further derive higher-order refinements that isolate a sampling-fraction skew term and characterize parity-driven second-order validity, with extensions to clustered designs and lattice outcomes.
\end{abstract}
...
\end{itemize}
We also show the deficiency bound is rate-optimal generically (and improves under symmetry cancellation), and we treat lattice outcomes as a deliberate stress test where periodicity obstructs second-order validity unless jittered.
The logic is: experiment equivalence $\Rightarrow$ sufficiency for bounded-loss decision problems under reduced information $\Rightarrow$ forced CLT-order pivot geometry; thus any regular reduced-information equal-tailed interval that departs from the Gaussian pivot forfeits CLT-order validity.
We highlight the near-census regime as a deliberate stress test, where finite-population skew terms can dominate and second-order behavior changes.
...
The remainder of the paper proceeds as follows. Section~2 introduces the finite-population framework and notation. Section~3 develops the third-cumulant decomposition and isolates the sampling-fraction skew. Section~4 establishes a uniform studentized Edgeworth expansion, which underpins the posterior expansion in Section~5 and the structural second-order validity results in Section~6. Section~7 formalizes the calibration operator. Section~8 and Section~9 present extensions to clustered designs and lattice outcomes. Section~10 establishes experiment equivalence and asymptotic sufficiency. Section~11 presents the full-likelihood objective Bayes alignment, and Section~12 concludes with discussion. The appendix contains the construction of Markov kernels and total-variation bounds underlying the equivalence, together with proof details for the Regime~B sharpness arguments. For orientation, the spine is: experiment equivalence $\Rightarrow$ reduced-information sufficiency $\Rightarrow$ forced CLT-order geometry; the intervening sections supply the higher-order refinements and extensions.
...
\end{assumption}
Higher-order expansions and the total-variation/deficiency bounds throughout the paper use the $4+\delta$ moment condition above; weakening the moment assumptions for CLT-order equivalence is plausible but would require different tools and is left open.
\begin{remark}[Moment boundary and rate sharpness]\label{rem:pareto_boundary}
A Pareto-type finite-population sequence with tail index $\alpha\in(2,3)$ has finite variance but infinite third moment. In this setting a CLT for the studentized mean can still hold, yet the Kolmogorov distance to the Gaussian limit decays at rate $N^{1-\alpha/2}$ rather than $N^{-1/2}$. Consequently, a uniform $m_N^{-1/2}$ deficiency bound cannot hold without the $4+\delta$ moment assumption. This does \emph{not} preclude $\Delta(\mathcal E_N,\mathcal G_N)\to 0$; it only limits the attainable rate. See the complementary note for a concrete construction.
\end{remark}
\begin{remark}\label{rem:pareto_boundary_tv}
In particular, the total-variation/deficiency control underlying Theorem~\ref{thm:sufficiency} need not hold at CLT order under Pareto tails with $\alpha\in(2,3)$.
\end{remark}
...
\subsection*{Near-census phase transition}
Standard i.i.d. asymptotics effectively take the sampling fraction to be $0$, masking the regime change highlighted by
Proposition~\ref{prop:rate_transition}. Near-census sampling ($1-p_N\asymp \mN^{-1/2}$) marks a genuine phase transition:
the design-induced skew becomes leading-order and drives the attainable calibration rate.
...
This extension serves as a stress test in high-leverage settings where bounded-cluster theory is silent.
...
In particular, when the periodic term is nonvanishing (Remark~\ref{rem:lattice_nonvanish}), generic second-order validity fails without jittering.
This is a hostile discrete setting; the continuity correction demonstrates that the experiment-theoretic conclusions
persist even under lattice effects.
...
\end{theorem}
The lattice expansion contains a periodic term of order $\mN^{-1/2}$; jittering removes this leading periodic component and recovers the continuous-case expansion to first order.
...
\end{remark}
\begin{remark}[Nonvanishing periodic term for Bernoulli outcomes]\label{rem:lattice_nonvanish}
For Bernoulli outcomes with success fraction $q\neq 1/2$, the lattice periodic term in~\eqref{eq:lattice-edgeworth} is generically nonvanishing. To avoid degenerate cancellations, we assume a nonvanishing subsequence on which $\sup_x|\Pi_N(x)|$ is bounded away from zero.
\end{remark}
We now turn to experiment equivalence and asymptotic sufficiency. This section delivers the paper's spine—experiment equivalence and reduced-information sufficiency—while the preceding sections build the higher-order machinery and extensions.
...
$(\hat\tau,\hat V)$. The reduction kernel is the natural map to $(\hat\tau,\hat V)$, while the reverse kernel synthesizes
a Gaussian Hájek projection with matched first two moments; see Appendix~\ref{app:suff}.
...
\begin{remark}[CLT vs.\ experiment equivalence]
A CLT for $(\hat\tau,\hat V)$ describes the marginal limit of a statistic. Experiment equivalence is stronger: it supplies Markov kernels that transfer risks for bounded-loss decisions between the randomization experiment and a Gaussian shift experiment. Thus the sufficiency/``forced geometry'' conclusions are experiment-level, not a restatement of asymptotic normality.
\end{remark}
...
\end{proposition}
The non-vanishing condition on $P_1$ holds whenever the standardized third-cumulant term (including the sampling-fraction component) is bounded away from zero, so the lower bound is generic.
...
Equivalently, within the regular reduced-information class, the Gaussian pivot is the unique admissible CLT-order limit.
...
We provide a focused stress test to illustrate the sampling-fraction obstruction and the effect of calibration. Figure~\ref{fig:stress_tests} reports coverage errors for skewed lognormal outcomes under CRA at treated fractions $p\in\{0.5,0.9\}$, comparing the Gaussian pivot, Cornish--Fisher correction, and the calibrated interval. The $\sqrt{N}$-scaled error is roughly flat for the Gaussian and Cornish--Fisher methods at $p=0.9$, consistent with $O(N^{-1/2})$ behavior, while the calibrated interval reduces the error to the Monte Carlo noise floor. At $p=0.5$ the errors are small for all methods, consistent with the predicted cancellation of the sampling-fraction skew.

\begin{figure}[t]
\centering
\includegraphics[width=0.90\linewidth]{../experiment/repro/plots/rate_grid/run_legacy/figs/rate_grid_tight_sqrt_log.png}
\vspace{0.4em}
\includegraphics[width=0.90\linewidth]{../experiment/repro/plots/rate_grid/run_legacy/figs/rate_grid_tight_linear_log.png}
\vspace{0.4em}
\includegraphics[width=0.62\linewidth]{../experiment/repro/plots/rate_grid/run_legacy/figs/rate_grid_tight_raw_p05_crop.png}
\caption{Stress-test coverage errors for skewed lognormal outcomes under CRA at $p\in\{0.5,0.9\}$. Top: $\sqrt{N}$-scaled error; middle: $N$-scaled error (both on a log scale to emphasize regime separation). Bottom: raw error at $p=0.5$, showing that differences among methods are near the Monte Carlo noise floor. Shaded bands are Monte Carlo standard errors. Near census ($p=0.9$), the Gaussian and Cornish--Fisher errors align with $O(N^{-1/2})$ scaling, while the calibrated interval reduces error to the simulation noise floor.}
...
Figure~\ref{fig:stress_tests} illustrates the regime distinctions as stress tests rather than as primary evidence: moderate sampling fractions versus near-census sampling, and calibrated versus uncalibrated intervals under skewed outcomes. The near-census panels contrast a standard pivot interval against the calibrated interval, highlighting the $O(\mN^{-1/2})$ gap predicted by the theory.
...
whose likelihood ratio matches that of the Gaussian shift up to $o(1)$ in total variation. Concretely, given $Z_N=(\hat\tau,\hat V)$, let
$\{\xi_i\}_{i=1}^N$ be i.i.d.\ $\mathcal N(0,1)$ and set
\[
\tilde U_i=\frac{\sqrt{\hat V}}{\sqrt{N}}\Big(\xi_i-\bar\xi\Big),\qquad \bar\xi=\frac1N\sum_{j=1}^N\xi_j,
\]
so that $\sum_i \tilde U_i=0$ and $\frac{1}{N}\sum_i \tilde U_i^2\approx \hat V$.
Fix a baseline sequence $\tilde Y_i^{(0)}$ with mean $\hat\tau$ and bounded fourth moments (e.g., a centered two-point sequence scaled to match $\hat V$),
and define the synthetic potential outcomes $\tilde Y_i(1)=\tilde Y_i^{(0)}+\tilde U_i$ and $\tilde Y_i(0)=\tilde Y_i^{(0)}$.
Given these synthetic outcomes, draw a CRA assignment $W$ to generate synthetic observed data. By construction, the induced studentized
statistic $\tilde S_N$ has the same first two moments as $S_N$ and satisfies the same uniform Edgeworth expansion as in
Theorem~\ref{thm:edgeworth}; thus $\|\mathcal L(\tilde S_N)-\mathcal N(0,1)\|_{\mathrm{TV}}=o(1)$ and the likelihood ratios match up to the
uniform remainder. Concretely, the uniform remainder implies a bound on $\sup_x|\Pr(\tilde S_N\le x)-\Phi(x)|$, which controls
the risk for bounded threshold losses and hence the total-variation deficiency at CLT order. This yields a reverse kernel $L_N$
with total-variation error $o(1)$.
