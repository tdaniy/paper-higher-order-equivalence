...
\documentclass[11pt]{article} % imsart.cls not available in this build environment

\usepackage{amsthm,amsmath,amssymb,mathtools,bm}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{enumitem}
...
\begin{document}
\title{\textbf{Higher-Order Equivalence of Bayesian and Randomization Inference in Finite Populations}}
\author{Talgat Daniyarov}
\date{}
\maketitle

\begin{abstract}
We establish the higher-order equivalence of Bayesian and randomization inference in finite populations by proving that the randomization experiment is asymptotically equivalent in Le Cam deficiency distance to a Gaussian shift experiment. This equivalence implies that the studentized design-based estimator is an asymptotically sufficient statistic for the experiment, forcing any regular, design-valid Bayesian procedure to align with the randomization geometry at CLT order. Beyond first order, we derive uniform studentized Edgeworth expansions that isolate a novel sampling-fraction skewness decomposition, identifying a qualitative transition in calibration requirements as the sampling fraction approaches a census. We further provide a Bayesian continuity correction via ancillary jittering for lattice outcomes and establish the sharp boundaries of second-order validity under high-leverage clustered designs.
\end{abstract}

\noindent\textbf{Keywords:} randomization inference; Bayesian calibration; finite populations; Edgeworth expansion; higher-order asymptotics.
...
The paper makes five main points, ordered by intellectual hierarchy. First, we prove that the randomization experiment is asymptotically equivalent, in Le Cam deficiency distance, to a Gaussian shift experiment whose central sequence is the studentized design-based estimator. Second, this experiment-level equivalence implies asymptotic sufficiency of the Gaussian pivot and yields a sharp CLT-order characterization: any regular design-valid equal-tailed Bayesian procedure must match the Gaussian pivot up to $o_p(m_N^{-1/2})$. Third, we show that finite-population randomization generates a \emph{sampling-fraction skew} (a design-induced asymmetry) term absent in i.i.d.\ asymptotics; near census sampling this term can dominate third-order behavior even under bounded moments. Fourth, we identify regime transitions in calibration scaling (near-census and high-leverage clustering in Regime~B), including a sharpness counterexample when maximal leverage is too large. Fifth, for lattice (binary) outcomes, we give a Bayesian-friendly ancillary jittering correction that removes periodic Edgeworth artifacts and restores $O(m_N^{-1})$ refined validity, and we provide numerical evidence corroborating the theoretical gains in non-asymptotic regimes where discreteness typically degrades coverage.

\paragraph{Main theorem in words.}
When validity is required under repeated randomization, the randomization experiment is asymptotically equivalent, in Le Cam's sense, to a Gaussian shift experiment whose central sequence is the studentized design-based estimator. At CLT order this equivalence forces Gaussian-pivot geometry for all regular design-valid equal-tailed Bayesian procedures. Beyond CLT order, finite-population combinatorics generate sampling-fraction skew and regime transitions that determine when refined calibration is automatic, when it requires explicit correction, and when it can fundamentally degrade.

The remainder of the paper proceeds as follows. Section 2 introduces the finite-population framework and notation. Section 3 develops the third-cumulant decomposition and isolates the sampling-fraction skew that distinguishes finite-population randomization from i.i.d. asymptotics. Section 4 establishes a uniform studentized Edgeworth expansion, which underpins the posterior expansion in Section 5 and the structural second-order validity results of Section 6. Section 7 extends the analysis to clustered designs, identifying calibration rate transitions and a sharpness boundary under high leverage. Section 8 addresses lattice outcomes and presents an explicit ancillary jittering correction that restores refined validity in discrete settings. Section 9 then develops the asymptotic sufficiency and deficiency-distance equivalence results, showing that the Gaussian pivot is forced at CLT order. Section 10 concludes with discussion. The appendix contains the construction of Markov kernels and total-variation bounds underlying the deficiency equivalence, together with proof details for the Regime B sharpness arguments.
...
Moreover, the finite-population variances that appear in the Hájek projection limits are uniformly nondegenerate: there exist constants $0<c<C<\infty$ such that the asymptotic variance $V$ of $m_N^{1/2}(\hat\tau-\tau)$ satisfies $c\le V\le C$, and the sampling fraction satisfies $\pi\in(0,1)$ in the sense that $N_1/N\to \pi$.
\end{assumption}
...
Theorem~\ref{thm:cumulant} separates superpopulation-style skewness from a genuinely finite-population component induced by assignment without replacement. Writing the treated fraction as $p=n_{1N}/N$ (equivalently $f=p$), the third cumulant admits the schematic decomposition
\[
\kappa_3\!\left(\frac{\hat\tau-\tau}{\sqrt{\Var(\hat\tau)}}\right)
=\text{(moment skew from }Y(1)\text{ and }Y(0)\text{)} \;+\; (1-2p)\times\text{(missing-mass skew from }\Delta\text{)} \;+\; o(m_N^{-1/2}).
\]
...
\begin{theorem}[Design-uniform Edgeworth expansion]
...
+ o(\mN^{-1})
\end{align*}
uniformly in $x$, for CRA, stratified, and bounded-cluster designs.
\end{theorem}
...
\begin{theorem}[Explicit calibration for asymmetric sets]
...
\section{Clustered designs with growing clusters (Regime B)}\label{sec:clusterB}
...
\section{Lattice outcomes and an explicit continuity correction}\label{sec:lattice}
...
\section{The Resolution---Randomization as a Constraint on Objective Bayes}\label{sec:sufficiency}


The preceding sections developed refined calibration for limited-information posteriors built from studentized design pivots. We now provide the paper's resolution: why a full-likelihood Bayesian---whose likelihood is \emph{ignorable} with respect to the assignment mechanism---is nonetheless trapped by randomization geometry. Ignorability makes the likelihood essentially flat in the unobserved potential outcomes, so any learning about the finite population must proceed through permutation-invariant (``objective'') structure on the space of units; this exchangeability, in turn, induces the same missing-mass uncertainty that governs randomization distributions. Our deficiency-distance equivalence results show that this alignment is not a coincidence of the difference-in-means estimator, but an unavoidable consequence of the experiment's information geometry: at CLT order the Gaussian pivot is asymptotically sufficient, forcing all regular design-valid equal-tailed Bayesian procedures to coincide with the pivot up to $o_p(m_N^{-1/2})$.

\begin{theorem}[Asymptotic sufficiency of $(\hat\tau,\hat V)$]\label{thm:sufficiency}
Under the CLT conditions of Theorem~\ref{thm:clt_characterization} and local alternatives $\tau=\tau_0+h/\sqrt{\mN}$,
the randomization experiment is asymptotically equivalent (in Le Cam's sense) to a Gaussian shift experiment observed
through $(\hat\tau,\hat V)$. In particular, $(\hat\tau,\hat V)$ is asymptotically sufficient at CLT order.
\end{theorem}
\begin{corollary}[CLT-order characterization as a consequence of sufficiency]\label{cor:char}
Any regular, first-order design-valid two-sided interval measurable with respect to $\sigma(\hat\tau,\hat V)$ satisfies
$C_{1-\alpha}=[\hat\tau\pm z_{1-\alpha/2}\sqrt{\hat V}]+o_p(\mN^{-1/2})$.
\end{corollary}


\section{Full-likelihood Objective Bayes and forced alignment with randomization}\label{sec:full_likelihood_forcing}
...
Here we show that, even if one begins with a full-likelihood Bayesian analysis over the
\emph{entire} finite-population vector of potential outcomes, permutation invariance and
diffuse nonparametric learning force posterior uncertainty to reproduce the
without-replacement geometry of randomization, including the finite-population correction.
...
$(Y_1(z),\dots,Y_N(z))\stackrel{d}{=}(Y_{\sigma(1)}(z),\dots,Y_{\sigma(N)}(z))$ under $\Pi_z$.
Moreover, conditional on $Y_{1:N}(z)$, the assignment is ignorable:
$\Prb(W\mid Y_{1:N}(0),Y_{1:N}(1))=\Prb(W)$.
We assume the moment/nondegeneracy conditions of Assumption~\ref{ass:global_moments}.
\end{assumption}
...
The joint prior $\Pi$ on $\{(Y_i(0),Y_i(1))\}_{i=1}^N$ is permutation invariant and,
conditional on the observed components, the posterior regression of a missing potential
outcome on its observed counterpart is $o_{\mathbb P}(N^{-1/2})$ uniformly:
for $z\in\{0,1\}$ and $i\in\mathcal O_z$,
\[
...
\begin{corollary}[Neyman-conservative forced alignment for ATE]\label{cor:neyman_conservative}
...
\paragraph{Step 1 (posterior for the finite-population measures).}
Under permutation invariance, learning about $Y_{1:N}(z)$ depends on the empirical
...
\paragraph{Step 2 (projection to $\tau_N$ and CLT-order normality).}
Write $\tau_N=\bar Y_N(1)-\bar Y_N(0)$ and use Proposition~\ref{prop:only_cross_term}.
...
\paragraph{Step 3 (second-order expansion; connection to Sections~\ref{sec:edgeworth}--\ref{sec:posterior}).}
The posterior for $\tau_N$ admits an Edgeworth/Cornish--Fisher expansion obtained by
...
Practically, the forcing viewpoint clarifies when departures from randomization are meaningful. Any attempt to obtain ``sharper'' posteriors for the ATE than the Neyman-conservative benchmark must introduce additional structure on cross-potential dependence; without it, conservative alignment is compelled. Conversely, when refined calibration fails (one-sided inference, lattice outcomes, or high-leverage clustering), the required higher-order adjustments can be interpreted as objective Bayes corrections that restore probability matching under the design.
...
Write $n_1=\sum_i W_i$, $n_0=N-n_1$, $p=n_1/N$, and the \emph{sampling fraction} $f:=n_1 n_0/N^2=p(1-p)$.
...
Write $\hat\tau=\sum_{i=1}^N a_i(W) \tau_i+\sum_{i=1}^N b_i(W)\{Y_i(1)-\bar Y(1)\}+\sum_{i=1}^N c_i(W)\{Y_i(0)-\bar Y(0)\}$,
...
\paragraph{Model and tangent space.}
Fix a sequence of finite populations $\mathcal P_N=\{(Y_i(1),Y_i(0))\}_{i=1}^N$ satisfying the regularity conditions.
...
\paragraph{Bernoulli explicit form.}
